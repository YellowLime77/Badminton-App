...
namespace BadmintonApp
{
    ...
    {
        public List<List<Object>> rallies;
        public Form1 frm1;
        ...
        private string GetNotesText(decimal rally, decimal shot)
        {
            DataFrame df = new DataFrame();
            foreach (List<Object> list in rallies)
            {
                if (list == rallies[(int)rally - 1])
                {
                    df = list[0] as DataFrame;
                }
            }

            return df.Rows[(int)shot - 1][7].ToString();
        }
        ...
        private void LoadChart1()
        {
            ...
            Chart chart1 = new Chart()
            {
                ...
                Size = new Size(tabPage4.Width, tabPage4.Height),
        ...
        {
            frm1 = Application.OpenForms.Cast<Form1>().Where(x => x.Name == "Form1").FirstOrDefault();
            if (frm1 != null)
            {
                Debug.WriteLine("LOADING FORM2 WITH FRM 1 OK");
                ...
                notesTextBox.Text = GetNotesText(rallyUpDown.Value, shotUpDown.Value);
            }
        }
        ...
        {
            DataFrame df = new DataFrame();
            List<Object> list = rallies[(int)rallyUpDown.Value - 1];
            df = list[0] as DataFrame;

            Debug.WriteLine(df.Rows[(int)shotUpDown.Value - 1][7]);
            df.Rows[(int)shotUpDown.Value - 1][7] = notesTextBox.Text;

            rallies[(int)rallyUpDown.Value - 1][0] = df;

            Debug.WriteLine(rallies);
        }
        ...
        private void endMediumPictureBox_SizeChanged(object sender, EventArgs e)
        {
            PictureBox box = sender as PictureBox;
            box.Width = 15;
            box.Height = 15;
        ...
        private void endSmallPictureBox_SizeChanged(object sender, EventArgs e)
        {
            PictureBox box = sender as PictureBox;
            box.Width = 7;
            box.Height = 7;
        ...
        private void endMediumPictureBox_LoadCompleted(object sender, AsyncCompletedEventArgs e)
        {
            PictureBox box = sender as PictureBox;
            box.Width = 15;
            box.Height = 15;
        ...
        private void endSmallPictureBox_LoadCompleted(object sender, AsyncCompletedEventArgs e)
        {
            PictureBox box = sender as PictureBox;
            box.Width = 7;
            box.Height = 7;
